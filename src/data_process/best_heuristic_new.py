...
def heuristic_bid_test(test_data, budget_para, base_bid_price, average_pctr):
    ...
    test_result= [np.sum(day_win_clk), average_pctr, budget_para, real_clks, bids, win_imps, real_imps, budget, day_spend,
...
def train_eval(config):
    if config.model == 'train':
        ...
        HB_bid_lin_PD.to_csv(config.HB_line_best_save_path + config.insert_type + '_best_{}_lin_bid_log.csv'.format(config.train_data_name),
                                    index=False)

        best_hb_base_bid_max = HB_bid_lin_PD.groupby(['budget_para']).apply(lambda x: x[x.win_pctr == x.win_pctr.max()])
        print('saving....')
        best_hb_base_bid_max.to_csv(config.HB_line_best_save_path + config.insert_type + '_best_{}_lin_bid.csv'.format(config.train_data_name),
                                    index=False)
        for budget_para in budget_proportions:
    ...
    pd.DataFrame(data=test_results,
                 columns=['all_win_clk', 'average_pctr', 'budget_para', 'real_clks', 'bids', 'win_imps', 'real_imps', 'budget', 'all_spend',
              'base_bid_price', 'win_pctr', 'end_time_all', 'day_win_clk', 'day_win_pctr',  'cpc_day', 'cmp_day', 'cpm', 'cpc'
                          , 'all_spend']).to_csv(
        config.HB_line_best_save_path + config.insert_type + '_best_{}_lin_day_bid.csv'.format(config.test_data_name), index=False)
...
class Config:
    def __init__(self):
        ...
        self.HB_line_save_path = self.result_path + 'HB/' + 'max_pctr_new_original/'
        self.HB_line_best_save_path = self.result_path + 'HB/' + 'max_pctr_new_original/' + self.insert_type + '/' + self.bid_type + '/'
